...
# Data source to check if the existing instance is available
data "aws_instance" "existing_instance" {
  count       = length(aws_instance.app_server.*.id) > 0 ? 1 : 0
...
resource "aws_instance" "app_server" {
  count = data.aws_instance.existing_instance.count > 0 ? 0 : 1
...
output "public_ip" {
  value = data.aws_instance.existing_instance.count > 0 ? data.aws_instance.existing_instance[0].public_ip : aws_instance.app_server[0].public_ip
}
